-- ACT Project Media Gallery System - SAFE VERSION
-- This version checks if tables exist and handles existing schemas gracefully

-- ============================================================================
-- 1. DROP EXISTING TABLES (if you want a clean start)
-- ============================================================================
-- Uncomment these lines if you want to completely recreate the tables:
-- DROP TABLE IF EXISTS project_media_links CASCADE;
-- DROP TABLE IF EXISTS video_embeds CASCADE;
...
DROP POLICY IF EXISTS "Public can read media_items" ON media_items;
DROP POLICY IF EXISTS "Public can read project_media_links" ON project_media_links;
DROP POLICY IF EXISTS "Public can read video_embeds" ON video_embeds;
DROP POLICY IF EXISTS "Authenticated users can insert media" ON media_items;
DROP POLICY IF EXISTS "Authenticated users can update their media" ON media_items;
DROP POLICY IF EXISTS "Authenticated users can delete their media" ON media_items;
DROP POLICY IF EXISTS "Authenticated users can manage media links" ON project_media_links;
DROP POLICY IF EXISTS "Authenticated users can manage video embeds" ON video_embeds;
...
DROP TRIGGER IF EXISTS update_video_embeds_updated_at ON video_embeds;
...
DO $$
BEGIN
  RAISE NOTICE 'Media gallery migration completed successfully!';
  RAISE NOTICE 'Tables created: media_items, project_media_links, video_embeds';
  RAISE NOTICE 'Next step: Create "media" storage bucket in Supabase dashboard';
END $$;
