...
- ~~`og:url` renders `http://localhost:3001`~~ — **fixed defensively in code.** `src/app/{sitemap,robots,layout}.ts` now guard against localhost/127.x/0.x in `NEXT_PUBLIC_SITE_URL` and fall through to `VERCEL_PROJECT_PRODUCTION_URL` → `VERCEL_URL` → canonical `act-regenerative-studio.vercel.app`. Verified `/sitemap.xml`, `/robots.txt`, and `/` `og:url`/`canonical` all now emit the prod URL even with the stale `.env.local` value. User should still clean `.env.local` for clarity but the site is no longer a shipping risk on this axis.
- ~~Closing-CTA variance across identity-family~~ — **mapped, not variance.** Three consistent patterns across the 10 identity pages: (1) **Dark callout family** (`bg-[#11110F] text-[#F3EBDD]` + gold `#CFA16B` button, identical template) on `/vision`, `/governance`, `/impact` — 3 pages. (2) **Content closer** (no CTA, ends on a grid of concepts/metaphors) on `/principles` (voice characteristics), `/how-we-work` (farm metaphor), `/studio` (impact preview), `/ask` (AskACT component). (3) **Bespoke** on `/about` (warm gradient `from-[#F6F1E7] via-[#E7DDC7] to-[#D7C4A2]` — but it's a *content* section, dual-entity structure, not a CTA) and `/partners` (sand/dashed `bg-[#F7F0E2]/70 border-dashed border-[#BFA883]` — funding-partnership specific). `/events` closes on content. The family is already coherent; no fix needed.
- **Wiki source content avoid-list residue — partial pass done.** Scanned `../act-global-infrastructure/wiki`: 37 hits across 24 files. Dropped 22 hits (meaning-carrying prose): `"living memory/practice of cultural sovereignty/belonging"` etc. on Barry, Marjoyie, Uncle Alan, Oonchiumpa, Campfire, Green Harvest Witta — these are evocative, correct descriptions of people and places, not UI-chrome jargon. Mechanically swapped 10 instances across 8 files where the phrase was system-architecture framing: `"public shell"` → `"public website"`, `"second CMS"` → `"redundant content store" / "content stores"`. Files touched (wiki repo, uncommitted): `technical/act-knowledge-ops-loop.md`, `technical/wiki-project-and-work-sync-contract.md`, `concepts/llm-knowledge-base.md`, `concepts/tractorpedia.md` (×2), `concepts/ways-of-working.md`, `decisions/continuous-pipeline.md`, `sources/2026-04-07-scrape-act-place-{about,empathy-ledger}.md`, `projects/act-studio/act-studio.md`. Regenerated snapshots via `npm run sync:wiki`; verified `/wiki/act-studio` now renders "public website" ×3 and zero "public shell". Remaining 15 hits: `concepts/living-website-operating-system.md` (12 — concept doc's core framing, needs author reframe not find/replace; also backlinked from 3 other docs), `index.md:43` nav description (refers to the concept doc by its published description), plus `operations/` and `output/` (not rendered on site).

### Session commit roll-up (most recent first)

- **(uncommitted, 2026-04-22)** Smoke walk + obvious-bug fixes on dev :3300:
  - **Favicon + web manifest.** Pre-fix: `/favicon.ico` 404'd (browsers request this by default regardless of the `<link rel="icon">` tag, so server logs were full of 404 noise); no manifest at all (no PWA install, no theme color hint, no Home Screen icon on iOS). Added `src/app/manifest.ts` (Next.js App Router convention — auto-serves at `/manifest.webmanifest` with `<link rel="manifest">` auto-injected in head; name/short_name, start_url, olive theme color `#4F6C3A`, sand background `#F6F1E7`). Copied `act-logo-square.png` → `public/favicon.ico` to satisfy default browser favicon requests. **Also verified:** zero `target="_blank"` links missing `rel="noopener"` (45 external links audited across 16 user-facing routes); `priority` image usage is all correct (heroes, logo, conditionally-rendered lightbox); next/font handles font preloading in prod builds.
  - **Keyboard + screenreader a11y.** Added skip-to-content link in `src/app/layout.tsx` (visually hidden until focus, jumps to `<main id="main-content">`). Labeled all 4 `<nav>` landmarks with `aria-label` so screen readers can distinguish them: Primary (header), Footer — quick links / fields of practice / studio (UnifiedFooter), Ecosystem — related sites (EcosystemLinks). Pre-fix: every page had "navigation" announced 4 times with no distinction. `html lang="en"` already correctly set.
  - **a11y/SEO landmark discipline.** Audited h1/main/footer counts across 25 routes. Fixed `/vision`: h1 was `2` (PageHero + `# ACT 2026: The Harvest Year` from embedded `vision.md`), `main` was `2` (layout.tsx + page-level `<main>` wrapping MarkdownViewer). Fix: strip leading `# Title` from markdown before render (vision.md h1 was also duplicating the visible SectionHeading title); swap inline `<main>` → `<div>`. `/goods` 2× `<footer>` turned out to be valid HTML5 (LeadVoice blockquote footer + global site footer) — not a bug. All 25 routes now at h1=1, main=1. **Added themed `/src/app/not-found.tsx`** — site was shipping Next.js's default bare "404 | This page could not be found." page. New 404 matches site palette (Warm Editorial, display font, olive accent) and offers four next-move cards (Home, Projects, Wiki, Contact).
  - **Sitemap + robots + og:url hardened against stale env.** `src/app/{sitemap,robots,layout}.ts` now filter out `localhost/127.x/0.x` values of `NEXT_PUBLIC_SITE_URL` and fall through to Vercel's auto-set `VERCEL_PROJECT_PRODUCTION_URL` → `VERCEL_URL` → canonical domain. Before fix: /sitemap.xml carried 570 `<loc>http://localhost:3001/*</loc>` entries and /robots.txt said `Host: http://localhost:3001` (would have told Google to crawl localhost on first Vercel deploy). After fix: all three endpoints emit `https://act-regenerative-studio.vercel.app/*` in dev. Production Vercel deploy will auto-pick `VERCEL_PROJECT_PRODUCTION_URL`.
  - **Full health pass.** Production build (`npm run build`): zero warnings, zero errors, all 90+ routes compile. Dev log: 1227 lines, zero runtime warnings/errors/hydration issues across the smoke walk. OG + Twitter metadata coverage: 25/25 top routes have full cards (og:title, og:description, og:image, twitter:card). **Note:** rendered `og:url` on `/` is `http://localhost:3001` (stale `NEXT_PUBLIC_SITE_URL` in `.env.local` — set correctly for Vercel before shipping). **Gotcha:** running `npm run build` while `next dev` is up clobbers the dev webpack cache and the dev server starts 500ing on routes until restarted — don't interleave.
  - **Full-site link audit — 0 broken links across 571 unique internal paths.** Crawled 25 top-level routes, extracted all internal `href`s (including preload links for `priority` images), hit each. One 404 found + fixed: `/empathy-ledger` hero was referencing missing `/media/field-stills/el-storytelling.jpg`. Swapped to `empathy-ledger-community-story.jpg` (same dir, matches alt text "storytelling session"). Re-audit clean.
  - **Code-comment drift cleaned.** `src/lib/empathy-ledger-services.ts:4` "ACT studio shell" → "ACT site". `src/lib/projects/get-project-data.ts:106,359` "Canonical ACT wiki" → "Working wiki" (2 occurrences). Remaining avoid-list hits are all inside `/admin/*` interior-ops dashboards — out of scope for external-voice rule.
  - **`/impact` stat-block colors neutralized.** 4 top stats now all use `var(--we-warm-brown)` (was orange/warm-brown/green/olive mix). Reasoning: the pre-existing color language is ambiguous — orange means "type" in Sankey but "authority" in SignalBars; green consistently means "evidence". Reinforcing the overloaded orange on the top stats would deepen the drift. Neutralizing lets the legend-carrying widgets below (Sankey + SignalBars) own the semantic color. Also removed unnecessary nested flex wrapper on the 4th block (inconsistent with the other three). If user prefers legend-aligned stats, revert to: evidence=`#4CAF50`, authority=`#D87D4A`, top two neutral.
  - **Obsidian `_template.md` files were shipping publicly.** Two `projects/act-farm/{case-studies,experiences}/_template.md` scratch files surfaced as `/wiki/_template` list entries (404 on click) and `/projects/_template` rendered as a live page with `<Case Study Title>` placeholder heading. Root cause: 3 separate wiki globs (`canonical-site-wiki.ts`, `canonical-project-wiki.ts`, plus sync scripts `sync-canonical-wiki-{pages,projects}.mjs`) did not exclude underscore-prefixed files. Fix: added `'**/_*.md'` to `ignore:` in all four glob calls (standard Obsidian/Jekyll convention for drafts); added defensive filter in `listAllWikiPages()` so upstream drift can't leak `_*` stems again; regenerated snapshots via `npm run sync:wiki`.
  - **Raw `[text](../sources/foo.md)` Obsidian links leaked into rendered HTML on `/wiki/<slug>`.** Example: `/wiki/empathy-ledger` was shipping 5× `href="../sources/*.md"` hrefs that 404'd from the browser. `renderCanonicalWikiMarkdown()` only handled `[[wikilink]]` syntax, not raw markdown `.md` links. Fix: extended regex to resolve raw `.md` targets via `matchPageRecord()` → `/wiki/<stem>`, preserving `#anchor` fragments; falls through to plain text when target doesn't resolve. Verified on `/wiki/empathy-ledger` — 4 raw hrefs now correctly rewritten to `/wiki/2026-04-07-el-*` routes.
  - **Stale "Canonical ACT wiki" eyebrow on `/wiki`** (`src/components/wiki/WikiSearch.tsx:129`) → "Working wiki" (matches Phase B.3 voice-pass decision).
  - **`LivingSystemStrip` had `eyebrow = "Living system"` as default** (avoid-list term). All 16 callers already pass explicit `eyebrow=`, so the default was dead code but a drift risk. Made prop required (removed default). `tsc --noEmit` clean.
  - Ledger entry `[ ] Wiki editing auth upgrade` struck as moot — commit `8b33a26` (Apr 20) removed `/wiki/[slug]/edit` entirely; Obsidian (filesystem resolver) is now the single source of truth for wiki pages, Supabase is warm fallback. No `WIKI_EDIT_TOKEN` references remain in `src/`.
...
- [x] ~~**Wiki editing auth upgrade**~~ — moot. Commit `8b33a26` removed `/wiki/[slug]/edit` entirely; Obsidian (filesystem resolver at `../act-global-infrastructure/wiki`) is the single source of truth, Supabase stays as warm fallback. `WIKI_EDIT_TOKEN` no longer referenced in `src/`.
